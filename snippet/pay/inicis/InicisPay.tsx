...
export interface InicisPayProps {
	isPurchase: boolean;
	isTest: boolean;
	payData: InicisPayData;
}

export interface InicisPayData {
...
const InicisPay: React.FC<InicisPayProps> = ({ payData, isPurchase, isTest }) => {
	...
	const onClickPurchase = () => {
		...
		} else if (mobilePurchaseRef.current) {
		...
		<div style={{ display: 'none' }}>
			...
			<form id='SendPayForm_id' name='' method='Post'>
				...
				<input type='hidden' readOnly name='signature'
					   value={SHA256(`oid=${payData.productId}&price=${payData.productPrice}&timestamp=${payData.timestamp}`)} />
				<input type='hidden' readOnly name='oid' value={payData.productId} />
				...
				{payData.payStatus === 2 &&
					<input type='hidden' readOnly name='acceptmethod' value={`HPP(${payData.telStatus})`} />}
			...
			<form name='mobileweb' method='post' acceptCharset='euc-kr' ref={mobilePurchaseRef}>
				...
				<input type='text' readOnly name='P_OID' value={payData.productId} />
				...
				{/* 휴대폰결제 필수 [1:컨텐츠, 2:실물] */}
				{payData.payStatus === 2 &&
					<input type='text' readOnly name='P_HPP_METHOD' value={payData.telStatus} />}
			</form>
